...
class DocApprovalDocumentPackage(models.Model):
    ...
    book_id = fields.Many2one("ii.edk.book", string="Workbook")
    document_type = fields.Many2one("ii.edk.document.type", string="Document Type")
    ...
    @api.model_create_multi
    def create(self, vals_list):
        _logger.info("PPPPPPPPPPP  vrednosti vals_list %s", vals_list)

        for vals in vals_list:
            if vals.get('partner_id'):
                kuf_id_sa_datim_brojem_fakture = self.env['ii.edk.document.package'].search(
                    [('document_ref', '=', vals.get('document_ref')),
                ...
                #            raise UserError(
                #                _('Faktura sa brojem %s za ovog dobavljača već postoji!')
                #                % i.document_ref
                #            )
            ...
            if vals.get('document_ref_internal'):
                for i in bill_id_sa_datim_brojem_fakture:
                    if i.document_ref_internal == vals.get('document_ref_internal'):
                        raise UserError(
                            _('Faktura sa internim brojem %s za ovog dobavljača već postoji!')
                            % i.document_ref_internal
                        )
        ...
        result = super(DocApprovalDocumentPackage, self).create(vals_list)
        return result
    ...
    def _compute_approval_state(self):
        for record in self:
            ...
            _logger.info("$$$$$$$ Nadjeni sledevi odobtavaoci %s", approvers)
            ...
            if len(approvers) == len(approvers.filtered(lambda a: a.state == 'approved')):
                ...
                # if sef_sinhro:
                if 'efaktura_api_url' in self.env['res.company']._fields:
    ...
    def get_current_approvers(self):
        self.ensure_one()
    #    _logger.info("PARTICIPANTS State= %s Step=%s", self.participant_ids[0].state, self.participant_ids[0].step)
        for participant in self.participant_ids:
            _logger.info("PARTICIPANT State= %s Step=%s", participant.state, participant.step)
        return self.participant_ids.filtered(lambda a: a.state == 'to approve' and a.step == self.approval_step)

    def get_current_approver(self):
        self.ensure_one()
        current_approvers = self.get_current_approvers()
        if not current_approvers:
            raise UserError(_('There are not approvers for this document package! -606'))
        ...
        for record in self:
            ...
            if record.approval_state == 'pending':
                ...
                if next_approvers:
                    ...
                    record.send_notification('ii_edk_base.request_to_approve', partner_ids)
                else:
                    raise UserError(_('There are not approvers for this document package!-650'))
    ...
    def approve_invoice_on_safe(self):
        ...
        #apikey = self.company_id.efaktura_api_key
        #
        _logger.debug('!!!!!!!!!   ---   Ovo je URL koji smo procitali = %s headers = %s', url,
...
class DocApprovalDocument(models.Model):
    ...
    def create(self, vals):
        ...
        if isinstance(vals, list):
            for v in vals:
                _logger.info("FFFFFFFFF nasao ID = %s", v.get('document_package_id'))
        else:
            _logger.info("FFFFFFFFF nasao ID = %s", vals.get('document_package_id'))
        for val in vals:
            p_ids = self.env['ii.edk.document.package'].search([('id', '=', val['document_package_id'])])
            _logger.info("Nasao ID = %s", val['document_package_id'])
...
class DocumentInvoceLine(models.Model):
    ...
    total_price = fields.Float(compute="_compute_line_total_price", store=True, string="Total Price")
    document_package_id = fields.Many2one('ii.edk.document.package', string='Rental options')
    tax_category_code = fields.Many2one("ii.pdv.category", string="Tax Category Code")
    tax_category_reason_code = fields.Many2one("ii.pdv.exemption.reason", string="Tax Category Reason Code", domain="[('category_id', '=', tax_category_code)]")
    tax_exemption_reason = fields.Char(string="Tax Exemption Reason")
