    ...
    def update_effective_date(self):
        for picking in self.env['stock.picking'].browse(self._context.get('active_ids', [])):
            ...
            if bool(account_move_short_code) == False:
                raise ValidationError('Account Stock Journal (Inventory Valuation Journal) for ' + str(
                    self.env.company.name) + ' is not identified yet! Go to Invoicing (Accounting) > Configuration > Default Effective Date Journal')
            ...
            purchase_orders_ids = picking.purchase_id
            ...
            if current_picking_id == 'internal':
                ...
                if bool(purchase_orders_ids):
                    if purchase_orders_ids.currency_id.id != system_default_currency:
                        ...
                        if rate == 0.0:
                            raise UserError('The currency rate for ' + str(purchase_orders_ids.currency_id.name) + ' of selected date ' + '(' + str(selected_date.strftime('%Y-%m-%d')) + '). is not available. Please update it under Accounting > Settings > Currencies > ' + str(purchase_orders_ids.currency_id.name) + '!')
                        else:
                            po_quantity = []
                            po_price_unit = []
                            po_tax_include = []
